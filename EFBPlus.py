...
# --- 1. Page Configuration (Spotify-like Dark UI) ---
        ...
        with s_tab1:
            ...
            with st.expander("ADD NEW LINK"):
                with st.form("add_link_form", clear_on_submit=True):
                    new_n = st.text_input("NAME"); new_u = st.text_input("URL")
                    if st.form_submit_button("ADD"):
                        quick_links.append({"name": new_n, "url": new_u})
                        with open(LINK_FILE, "w", encoding="utf-8") as f: json.dump(quick_links, f)
                        st.rerun()

        with s_tab2:
            menu = st.radio("SELECT TOOL", ["PILOT LOCATIONS", "TIMERS", "OFP", "T/D CALC", "TURN RADIUS", "PAD", "WEATHER", "LOG", "CONVERTER", "X-WIND", "VATSIM"])
    ...
    with main_tab1:
        if menu == "PILOT LOCATIONS":
            ...
            with c1:
                ...
                if st.button("START SW"): st.session_state['sw_start_time'] = time.time(); st.session_state['sw_running'] = True
                if st.button("RESET SW"): st.session_state['sw_running'] = False; st.session_state['sw_start_time'] = 0
            ...
            if st.button("FETCH SIMBRIEF"):
                ...
                if res.status_code == 200:
                    st.session_state['sb_json'] = res.json(); st.success("LOADED"); st.rerun()
            ...
            cur = c1.number_input("Cur Alt", 0, 45000, 35000); tgt = c2.number_input("Tgt Alt", 0, 45000, 3000); gs = c3.number_input("GS", 100, 600, 400)
            st.metric("Dist NM", f"{((cur - tgt) / 1000) * 3:.1f}")
            st.metric("Req VS", f"-{gs * 5}")

        elif menu == "PAD":
            st.subheader("SCRATCH PAD")
            st_canvas(fill_color="rgba(255, 165, 0, 0.3)", stroke_width=3, stroke_color="#FFFFFF", background_color="#121212", height=400, key="canvas")
            ...
            v_icao = st.text_input("AIRPORT ICAO", "RJTT").upper()
            v_res = requests.get("https://data.vatsim.net/v3/vatsim-data.json")
            if v_res.status_code == 200:
                v_data = v_res.json()
                pilots = [p for p in v_data.get("pilots", []) if (p.get("flight_plan", {}) or {}).get("arrival") == v_icao]
                for p in pilots: st.info(f"**{p['callsign']}** ➔ {v_icao}")
    ...
    with main_tab3:
        ...
        if os.path.exists(DB_FILE):
            ...
            for idx, entry in enumerate(reversed(all_logs)):
                with st.expander(f"{entry['date']} | {entry.get('route', 'N/A')} - {entry.get('maint_status', 'PENDING')}"):
                    if entry.get("maint_status") == "PENDING" and st.button(f"RELEASE (IDX:{idx})", key=f"mbtn_{idx}"):
                        all_logs[len(all_logs)-1-idx]["maint_status"] = "RELEASED"
                        with open(DB_FILE, "w", encoding="utf-8") as f: json.dump(all_logs, f); st.rerun()
